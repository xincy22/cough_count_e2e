...
from coughcount.runtime import tqdm_disabled
        ...
        try:
            with requests.get(
                ...
                if int(r.status_code) == 416:
                    # Server reports "range not satisfiable" which usually means file is complete.
                    return

                if resume_pos > 0 and int(r.status_code) == 200:
                    # Range not honored; restart from scratch to avoid duplicated bytes.
                ...
                total_size = (
                    int(total) + resume_pos
                    if (total is not None and resume_pos > 0)
            ...
            wait_s = float(retry_backoff_sec) * min(float(attempts), 5.0)
            print(
                f"Download interrupted (attempt {attempts}/{max_retries}): {e}. "
...
def ensure_edgeai_downloaded(edgeai_raw_dir: Path) -> Path:
    ...
    if got.lower() != EXPECTED_MD5.lower():
        print(
            "Detected incomplete/corrupted zip checksum, "
    ...
    if got.lower() != EXPECTED_MD5.lower():
        print("Checksum still mismatched, restarting download from scratch...")
    ...
    if got.lower() != EXPECTED_MD5.lower():
        raise RuntimeError(
            f"MD5 mismatch after retries: expected {EXPECTED_MD5}, got {got}\n"
            f"please verify network integrity and retry."
        )
