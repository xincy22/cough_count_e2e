...
class EdgeAIWindowDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        normalize: str = "none",  # "none" | "per_sample" | "per_freq"
        ...
        self.splits_json = Path(splits_json) if splits_json is not None else P.edgeai_splits_json
        self.subjects = self._load_split_subjects(split, self.splits_json)
        ...
        for si, sp in enumerate(self.samples):
            self._data_cache[si] = self._load_sample_arrays(sp)
    ...
    def _normalize_S(self, S: np.ndarray) -> np.ndarray:
        ...
        if self.normalize == "per_freq":
            ...
            std = np.std(S, axis=1, keepdims=True)
    ...
    def _load_sample_arrays(
        ...
        t = np.load(sp.t_path).astype(np.float32)
        return S, density, t
