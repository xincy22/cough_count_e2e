...
def sample_count_error(
    ...
    *,
    ...
    """
    Per-sample count error used for training, shape [B].

    mode:
      - "mae": abs(pred_count - tgt_count)
      - "rel_mae": abs(pred_count - tgt_count) / (tgt_count + rel_eps)
      - "log1p_mae": abs(log1p(pred_count) - log1p(tgt_count))
    """
    mode = str(mode).lower()
    _, tsz = pred.shape
    mask = _build_valid_mask(lengths, tsz, pred.device)
    ...
    if mode == "mae":
        return (pred_count - tgt_count).abs()
    if mode == "rel_mae":
        denom = (tgt_count + float(rel_eps)).clamp_min(1e-6)
        return (pred_count - tgt_count).abs() / denom
    if mode == "log1p_mae":
        # Density can go slightly negative early; keep log stable.
        pred_c = pred_count.clamp_min(0.0)
        tgt_c = tgt_count.clamp_min(0.0)
        return (pred_c.log1p() - tgt_c.log1p()).abs()
    raise ValueError(f"Unknown count error mode: {mode}")
...
def train_loss_weighted(
    ...
    *,
    count_loss_weight: float = 0.1,
    count_loss_mode: str = "mae",
    count_rel_eps: float = 1.0,
    ...
    if float(count_loss_weight) > 0.0:
        count_each = sample_count_error(
            pred,
            target,
            lengths,
            mode=str(count_loss_mode),
            rel_eps=float(count_rel_eps),
        )
