"""
训练脚本 - 验证最佳density和loss组合
从experiment.yaml读取配置，训练所有组合配置
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train all combination variants.")
    parser.add_argument(
        "--combo-id",
        type=str,
        default=None,
        help="Train only a specific combination (e.g., '3A', '3B', etc.)",
    )
    parser.add_argument(
        "--epochs",
        type=int,
    ...
    if args.combo_id:
        combinations = [c for c in combinations if c["id"] == args.combo_id]
        if not combinations:
            print(f"Error: combo_id '{args.combo_id}' not found")
            return

    results = []

    for combo_cfg in combinations:
        combo_id = combo_cfg["id"]
        combo_name = combo_cfg["name"]
        data_dir = base_data_dir / combo_cfg.get("data_dir", "gaussian_sigma50ms")

        if not data_dir.exists():
            print(f"Warning: data directory {data_dir} does not exist, skipping {combo_id}")
            continue

        # Prepare config for this combination
        combo_run_cfg = copy.deepcopy(cfg)
        combo_run_cfg["data"]["npy_dir"] = str(data_dir.resolve())
        combo_run_cfg["data"]["density_kernel"] = {
            "kernel": combo_cfg["kernel"],
            "sigma_sec": combo_cfg["sigma_sec"],
            "sigma_left_sec": combo_cfg.get("sigma_left_sec", 0.04),
            "sigma_right_sec": combo_cfg.get("sigma_right_sec", 0.08),
        }
        combo_run_cfg["training"]["count_loss_weight"] = combo_cfg["count_loss_weight"]
        combo_run_cfg["training"]["under_count_weight"] = combo_cfg.get("under_count_weight", 0.0)
        combo_run_cfg["training"]["under_count_positive_only"] = combo_cfg.get("under_count_positive_only", True)

        if args.epochs is not None:
            combo_run_cfg["training"]["epochs"] = int(args.epochs)
        if args.device is not None:
            combo_run_cfg["training"]["device"] = str(args.device)

        run_dir = runs_dir / combo_id
        run_dir.mkdir(parents=True, exist_ok=True)

        print(f"\n{'='*60}")
        print(f"Combination {combo_id}: {combo_name}")
        print(f"{'='*60}")
        print(f"Data: {data_dir}")
        print(f"Kernel: {combo_cfg['kernel']}")
        print(f"count_loss_weight: {combo_cfg['count_loss_weight']}")
        print(f"under_count_weight: {combo_cfg.get('under_count_weight', 0.0)}")
        print(f"Run dir: {run_dir}")
        print(f"Description: {combo_cfg['description']}")

        train_summary = train_single_run(combo_run_cfg, run_dir=run_dir, resume=resume)

        # Evaluate on test set
        eval_batch = int(combo_run_cfg.get("loader", {}).get("batch_size", 16))
        eval_workers = int(combo_run_cfg.get("loader", {}).get("num_workers", 4))
        eval_device = str(combo_run_cfg.get("training", {}).get("device", "cuda"))
        ...
        result = {
            ...
            "combination": combo_cfg,
            "train_summary": train_summary,
            "test_metrics": test_metrics,
        }
        results.append(result)

        print(f"\n[{combo_id}] Test results:")
    ...
    summary_path = runs_dir / "combination_summary.json"
    atomic_write_json(summary_path, summary)

    # Print comparison table
    print(f"\n{'='*80}")
    print("COMBINATION COMPARISON SUMMARY")
    print(f"{'='*80}")
